...
def log(
    ...
    if losses is not None:
        ...
        logger.add_scalar("Loss/kl_loss_text", losses[3], step) #modified
...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path):
    ...
    basenames = targets[0]
    # path_hifigan = os.path.join(path, 'hifigan')
    ...
    for d, wav, basename in zip(predictions[7], wav_predictions, basenames):
        ...
        # gf_save_path = os.path.join(path_gf, "{}.wav".format(basename.replace(' ', '_')))
        # gf_wav = audio.inv_melspectrogram(mel.cpu().numpy())
        # audio.save_wav(gf_wav, gf_save_path)
