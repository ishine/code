...
from utils.acl_vae_d_bert_tools import get_mask_from_lengths, pad
...
class VAE(nn.Module):
    ...
    def forward(self, mel_mean, output, durations): #modified
        output_in = durations.unsqueeze(-1)

        vae_in = torch.cat((output, mel_mean, output_in), -1)
...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        if mean_mels==None:
            ...
            vae_embedding, mu, logvar, text_mu, text_logvar = self.VAE(mean_mels, x_vae, duration_rounded)  # modified
