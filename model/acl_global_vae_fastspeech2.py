...
from .acl_global_vae_modules import VarianceAdaptor, VAE
from utils.acl_global_vae_tools import get_mask_from_lengths
...
class FastSpeech2(nn.Module):
    ...
    def forward(
        ...
        if mean_mels==None:
            print('inference-ing')
            vae_embedding, mu, logvar = self.VAE.inference(texts.size()) #modified
        else:
            vae_embedding, mu, logvar = self.VAE(mean_mels, output) #modified
