...
from utils.acl_prior_vae_tools import get_mask_from_lengths, pad
...
class VAE(nn.Module):
    ...
    def __init__(self, config):
        ...
        vae_in_dim = config['vae']['in_dim'] # input dim to vae
        self.vae_dim = config["vae"]["dim"] # vae output dim
        vae_up_dim = config["vae"]["up_dim"] # the dim of upsample

        self.mu_fc = nn.Conv1d(in_channels=vae_in_dim, out_channels=self.vae_dim, kernel_size=1)
        self.logvar_fc = nn.Conv1d(in_channels=vae_in_dim, out_channels=self.vae_dim, kernel_size=1)
        self.up_fc = nn.Conv1d(in_channels=self.vae_dim, out_channels=vae_up_dim, kernel_size=1)

        self.text_mu_fc = nn.Conv1d(in_channels=256, out_channels=self.vae_dim, kernel_size=1)
        self.text_logvar_fc = nn.Conv1d(in_channels=256, out_channels=self.vae_dim, kernel_size=1)

    def reparameterize(self, mu, logvar, text_mu, text_logvar): #modified
    ...
    def forward(self, mel_mean, output): #modified
        output_ = output.permute(0, 2, 1)
        ...
        vae_in = torch.cat((mel_mean, output), -1)
    ...
    def inference(self, input_shape, output):
...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        if mean_mels is not None:
            mean_mels, _ = self.length_regulator(mean_mels, duration_rounded, max_len)
        return (
            x,
            mean_mels,
