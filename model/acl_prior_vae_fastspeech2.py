...
from .acl_prior_vae_modules import VarianceAdaptor, VAE
from utils.acl_prior_vae_tools import get_mask_from_lengths
...
class FastSpeech2(nn.Module):
    ...
    def forward(
        ...
        if mean_mels==None:
            print('inference-ing')
            vae_embedding, mu, logvar, text_mu, text_logvar = self.VAE.inference(texts.size(), output) #modified
        else:
            vae_embedding, mu, logvar, text_mu, text_logvar = self.VAE(mean_mels, output) #modified
